...
export const validationExceptionFactory = (errors: ValidationError[]) => {
  ...
  const propertySpecificMapping = VALIDATION_EXCEPTION_MAPPINGS[errorKey as keyof typeof VALIDATION_EXCEPTION_MAPPINGS];
  if (propertySpecificMapping) {
    return propertySpecificMapping(property);
  }
  
  // 2. Then try generic constraint mapping
  const genericMapping = VALIDATION_EXCEPTION_MAPPINGS[constraintKey as keyof typeof VALIDATION_EXCEPTION_MAPPINGS];
  if (genericMapping) {
