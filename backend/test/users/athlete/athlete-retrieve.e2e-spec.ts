...
describe('Athlete profile (GET) (e2e)', () => {
  ...
  let successTests: Array<[string, string, string]> = [
    ...
    [
      `Successful with table, nested, and direct query`,
      'id, users (name), federations (*)',
      '?data=id,users.name,federations',
    ],
  ];
  ...
  test.each(failureFromQueryTests)(
    '%s',
    async (testName: string, query: string, invalidQuery: string) => {
      const res = await request(app.getHttpServer())
